...
export type SessionPhase = 'idle' | 'reading' | 'thinking' | 'writing' | 'warning' | 'completed'
...
const defaultConfig: SessionConfig = {
  ...
  write_time_seconds: 300, // 5 minutes
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  startSession: (sessionId, question) => set({
    sessionId,
  ...
  setPhase: (phase) => {
    ...
    switch (phase) {
      ...
      case 'writing':
        ...
        break
      case 'warning':
        timeRemaining = config.warning_time_seconds
        break
    }
  ...
  completeQuestion: (questionId) => set((state) => ({
